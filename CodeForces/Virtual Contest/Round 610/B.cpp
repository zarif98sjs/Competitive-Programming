...
So dp[i] = cost of item i + dp[i-k]. If dp[i] <= budget, our answer is atleast i.

If i < k the case is simpler, dp[i] is just the prefix sum
...
int main()
{
    ...
    while(tc--)
    {
        ...
        for(LL i=1; i<=n; i++)
        {
            if(i<k) dp[i] = dp[i-1] + ara[i];
            if(i>=k) dp[i] = dp[i-k] + ara[i];
            if(dp[i]<=p) ans = i;
        }
