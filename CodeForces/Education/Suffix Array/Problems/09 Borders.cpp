...
void printAllSubstrings(string s,const vector<int> &p,const vector<int> &lcp)
{
    for(int i=0; i<(int)p.size(); i++)
    {
        string temp = s.substr(p[i],s.size()-p[i]);

        for(int j=0;j<temp.size();j++)
            cout<<temp.substr(0,j+1)<<endl;
        cout<<endl;
    }
}

void Borders(string s)
{
    int n = s.size();
...
void printAllBorders(string s,const vector<int> &p,const vector<int> &lcp)
{
    for(int i=0; i<(int)p.size(); i++)
    {
        string temp = s.substr(p[i],s.size()-p[i]);
...
int main()
{
    ...
    DEBUG(s,p,lcp);
    printAllSubstrings(s,p,lcp);
    printAllBorders(s,p,lcp);
