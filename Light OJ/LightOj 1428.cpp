...
Suffix Array + KMP / Binary Search on Suffix Array
...
Now that we have solved all the easier versions . Let's solve our problem .

How many among the distinct substrings doesn't contain another string as a substring ?
- Let's call this string SUB  .
What's the thing that will change in our substring count ?
For any suffix , the ending position for possible substrings still starts from p[i] + LCP(i) . But this time we can't end at s.size()-1 as
that could lead to have substring having the string SUB. So , we need to find the new ending positions .

For this part we need to know the positions where our string SUB appears in our string S . We can use KMP for that . Also we can binary search on suffix array for that .
...
For , 0 it is 0+2 = 2 . So , we can make "a" "ab" but not "aba"
