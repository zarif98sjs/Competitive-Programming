...
void push(int v,int szl,int szr)
{
    ...
    Tree[lc].sum += szl*Tree[v].lazy; /** Multiplying the size of the left node with the parent's lazy value **/
    Tree[lc].lazy += Tree[v].lazy;

    Tree[rc].sum += szr*Tree[v].lazy; /** Multiplying the size of the right node with the parent's lazy value **/
...
void update(int cur, int start, int end, int l,int r, LL add)
{
    ...
    if (l==start && r==end)
    {
        Tree[cur].sum += (r-l+1)*add; /** Multiplying the size of the current node **/
        Tree[cur].lazy += add;
    }
    else
    {
        ...
        push(cur,mid-start+1,end-mid); /** Pushdown to children node **/
