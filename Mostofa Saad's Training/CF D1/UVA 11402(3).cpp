...
void node_update(int cur,int st,int en,int type)
{
    ...
    else if(type == 1) Tree[cur].sum = (en-st+1); /// assign 1
    else if(type == 2) Tree[cur].sum = (en-st+1) - Tree[cur].sum; /// flip

    int &ct = Tree[cur].lazy_type;
...
void update(int cur, int start, int end, int l,int r, int type)
{
    ...
    else
    {
        ...
        update(lc, start, mid,l,min(r,mid), type);
        update(rc, mid+1, end,max(l,mid+1),r, type);
